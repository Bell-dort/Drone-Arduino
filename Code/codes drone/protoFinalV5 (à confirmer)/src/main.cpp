...
void testJoystick()
{
  ...
  for(int i=0; i<4; i++)
  {
    if(puissanceCommune  < pMotorMin)
    {
      ...
      esc[i].writeMicroseconds(puissanceCommune);
    }
    else if(puissanceCommune  > pMotorMax)
    {
    ...
    {
      puissanceCommune = puissanceCommune + joyStickLeftY/30;
      esc[i].writeMicroseconds(puissanceCommune);
    }
